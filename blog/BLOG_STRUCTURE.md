...
- **Wide layout (≥1240px viewport):** the reading column grows to `868px` (≈820px of text, a readable ~75-character measure). The banner, grid-based `.figure` blocks, `<pre>` and `<table>` **full-bleed** to `1180px` via negative margins — they escape the column without touching the text flow. Figures whose only child is an `<svg>` do NOT bleed: a fixed `viewBox` just scales the diagram text ~2× without adding information (`.figure:has(> svg)` opts out). Below 1240px nothing changes. Copy the `@media (min-width: 1240px)` block verbatim from any existing post. Do NOT center prose children with `margin-left:auto` — element-qualified rules like `h1.article-title{margin:...}` outrank `.article > *` and the layout desyncs.
