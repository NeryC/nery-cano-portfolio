...
- **Wide layout + fluid type (≥1240px viewport).** Do NOT widen the column at a fixed font size: the reading column is already at ~91 characters per line at 19px, which is the ceiling. Instead, **scale the type with the viewport** so the column grows in proportion and the measure stays between 92 and 97 characters. Four steps: `20px/920px`, `21.5px/1000px`, `23px/1090px`, `25px/1200px` at `1240 / 1600 / 2000 / 2400px`. Sizes inside `.article` are expressed in `em` so they follow. The banner, grid-based `.figure` blocks, `<pre>` and `<table>` **full-bleed** to `1360 → 1880px` (capped at `94vw`) via negative margins, escaping the column without touching the text flow. Figures whose only child is an `<svg>` do NOT bleed: a fixed `viewBox` just scales the diagram without adding information (`.figure:has(> svg)` opts out). Below 1240px nothing changes. Copy the four `@media` blocks verbatim from any existing post. Never center prose children with `margin-left:auto`: element-qualified rules like `h1.article-title{margin:...}` outrank `.article > *` and the layout desyncs.
